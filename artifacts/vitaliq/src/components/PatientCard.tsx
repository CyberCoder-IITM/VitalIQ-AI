...
function VitalChip({ label, value, unit, critical }: { label: string; value: string | number; unit: string; critical: boolean }) {
  return (
    <div className={`rounded p-1 text-center ${critical ? "bg-red-500/15" : "bg-background/60"}`}>
      <div className={`text-xs font-bold leading-none font-mono ${critical ? "text-red-400 animate-cardiac-pulse" : "text-foreground"}`}>{value}</div>
...
export default function PatientCard({ patient, vitals, onClick }: Props) {
  ...
  const news2History = useRef<number[]>([]);
  const news2Score = news2?.score ?? 0;
  const [, forceUpdate] = useState(0);

  useEffect(() => {
    news2History.current = [...news2History.current, news2Score].slice(-8);
    forceUpdate((n) => n + 1);
  }, [news2Score]);
  ...
  const criticalAlerts = (alerts ?? []).filter((a: any) => !a.acknowledged && (a.severity === "CRITICAL" || a.severity === "URGENT")).slice(0, 1);
  ...
  const edHours = patient.arrival_time ? (Date.now() - new Date(patient.arrival_time).getTime()) / 3600000 : 0;
  const edDisplay = patient.arrival_time ? `${Math.floor(edHours)}h ${Math.floor((edHours % 1) * 60)}m` : "";
  const edColor = edHours > 4 ? "text-red-400" : edHours > 2 ? "text-amber-400" : "text-green-400";

  const borderClass =
    news2Score >= 7 ? "border-red-500/70 shadow-[0_0_10px_rgba(239,68,68,0.3)] animate-news-critical"
    : news2Score >= 5 ? "border-amber-500/60"
    : "border-border";

  const news2BadgeClass =
    news2Score >= 7 ? "bg-red-500/20 text-red-400 border border-red-500/40"
    : news2Score >= 5 ? "bg-amber-500/20 text-amber-400 border border-amber-500/40"
    : "bg-green-500/20 text-green-400 border border-green-500/40";

  const icuPct = icu?.percentage ?? 0;
  const icuColor = icuPct >= 75 ? "text-red-400" : icuPct >= 50 ? "text-amber-400" : icuPct >= 25 ? "text-blue-400" : "text-green-400";

  const isCrit = (val: number, lo: number, hi: number) => val < lo || val > hi;

  return (
    <div
      className={`rounded-lg bg-card border ${borderClass} p-3 cursor-pointer hover:brightness-110 hover:scale-[1.005] transition-all duration-200 select-none`}
      onClick={onClick}
    >
      <div className="flex items-start justify-between mb-2">
        <div>
          <div className="flex items-center gap-1.5 mb-0.5">
            <span className={`text-[9px] font-bold rounded px-1.5 py-0.5 ${ACUITY_COLORS[patient.acuity] ?? ACUITY_COLORS[3]}`}>ESI {patient.acuity}</span>
          ...
          {edDisplay && <div className={`text-[9px] mt-0.5 ${edColor}`}>⏱ {edDisplay}</div>}
        </div>
        <div className="flex flex-col items-end gap-1 shrink-0 mt-0.5">
          <div className="flex items-center gap-1">
            {unackAlerts > 0 && (
              <span className="bg-red-500 text-white text-[9px] font-bold rounded-full min-w-[16px] h-4 flex items-center justify-center px-1">{unackAlerts}</span>
            )}
            {majorInteractions > 0 && (
              <span className="bg-amber-500/20 text-amber-400 text-[9px] font-bold rounded border border-amber-500/40 px-1 h-4 flex items-center">⚠</span>
            )}
          </div>
          <span className={`text-base leading-none ${trendColor}`} title={trendTitle}>{trend}</span>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-1 mb-2">
        <VitalChip label="HR" value={vitals?.heart_rate?.toFixed(0) ?? "--"} unit="bpm"
          critical={!!vitals && isCrit(vitals.heart_rate, 50, 110)} />
        ...
        <VitalChip label="RR" value={vitals?.respiratory_rate?.toFixed(0) ?? "--"} unit="/min"
          critical={!!vitals && isCrit(vitals.respiratory_rate, 12, 20)} />
      ...
      {criticalAlerts.length > 0 && (
        <div className="mb-1.5 bg-red-500/15 border border-red-500/30 rounded px-2 py-0.5 text-[9px] text-red-300 truncate">
      ...
      <div className="flex items-center justify-between pt-1 border-t border-border/50">
        <span className={`text-[10px] font-bold rounded px-1.5 py-0.5 ${news2BadgeClass}`}>
          NEWS2 {news2Score}{news2Score >= 7 && <span className="ml-1 animate-pulse">●</span>}
        </span>
        <div className="flex items-center gap-1">
